...
![xxx](./assets/images/1.0d533687.png)
...
## 二.链表的反转
### (1)递归实现
![xxx](./assets/images/2.7d91fcef.png)
```js
reverseList(){
...
```
### (2)循环实现
![xxx](./assets/images/3.7b7f7fde.png)
```js
reverseList(){
    let head = this.head;
    if(head == null || head.next == null) return head;
...
```
## 三.单向循环链表
![xxx](./assets/images/4.e885222e.png)
```js
add(index, element) {
    if (arguments.length === 1) {
        element = index;
...
```
## 四.双向链表
![xxx](./assets/images/5.5e323416.png)
```js
class Node {
...
```
## 五.双向循环链表
![xxx](./assets/images/6.29e4fa47.png)
```js
class Node {
    constructor(element, prev, next) {
        this.element = element;
...
```
### 六.环形链表
![xxx](./assets/images/7.eeff2690.png)
